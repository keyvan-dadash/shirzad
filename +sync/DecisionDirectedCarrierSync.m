classdef DecisionDirectedCarrierSync < handle
    ...
    methods
        ...
        function y = process(obj, x)
            ...
            % fprintf('freq is %.3f and phase is %.3f\n', obj.freq, obj.phase);
        end
    end
    ...
    methods (Access = private)
        ...
        function d = decision(obj, y)
            ...
            if M == 4
                ...
                if re >= 0, reHat = 1; else, reHat = -1; end
                if im >= 0, imHat = 1; else, imHat = -1; end

                d = (reHat + 1j*imHat) / sqrt(2);

            else
                % Square QAM with average Es ≈ 1
                L = sqrt(M);
                ...
                reHat = sync.DecisionDirectedCarrierSync.quantize(real(y), levels);
                imHat = sync.DecisionDirectedCarrierSync.quantize(imag(y), levels);
    ...
    methods (Static, Access = private)
        function vq = quantize(x, levels)
            ...
            x      = x(:);          % column (N x 1)
            levels = levels(:).';   % row   (1 x L)

            [~, idx] = min(abs(x - levels), [], 2);  % NxL -> min over columns
            vq = levels(idx).';                      % Nx1
        end
    end
end
